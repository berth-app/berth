...
pub fn event_subject(agent_id: &str, event_type: &str) -> String {
    ...
    format!("runway.{agent_id}.event.{category}")
}

pub fn log_subject(agent_id: &str, project_id: &str) -> String {
    format!("runway.{agent_id}.log.{project_id}")
}

pub fn heartbeat_subject(agent_id: &str) -> String {
    format!("runway.{agent_id}.heartbeat")
}
...
pub fn cmd_subject(agent_id: &str, cmd_type: &str) -> String {
    format!("runway.{agent_id}.cmd.{cmd_type}")
}

pub fn resp_subject(agent_id: &str, request_id: &str) -> String {
    format!("runway.{agent_id}.resp.{request_id}")
}
...
mod tests {
    ...
    fn test_event_subjects() {
        assert_eq!(event_subject("vps1", "deploy_completed"), "runway.vps1.event.deploy");
        assert_eq!(event_subject("vps1", "execution_completed"), "runway.vps1.event.execution");
        assert_eq!(event_subject("vps1", "schedule_triggered"), "runway.vps1.event.schedule");
        assert_eq!(event_subject("vps1", "agent_upgraded"), "runway.vps1.event.agent");
    }

    #[test]
    fn test_log_subject() {
        assert_eq!(log_subject("vps1", "proj-123"), "runway.vps1.log.proj-123");
    }

    #[test]
    fn test_heartbeat_subject() {
        assert_eq!(heartbeat_subject("vps1"), "runway.vps1.heartbeat");
    }
