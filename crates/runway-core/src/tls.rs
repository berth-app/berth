...
pub fn generate_ca() -> Result<CertifiedIssuer<'static, KeyPair>> {
    ...
    let mut params = CertificateParams::new(vec!["Runway CA".to_string()])?;
    ...
    params
        .distinguished_name
        .push(rcgen::DnType::CommonName, "Runway CA");
...
///
/// Used by the Runway app to authenticate itself to remote agents via mTLS.
...
/// Returns the directory where Runway stores TLS certificates.
///
/// Resolves to `~/Library/Application Support/com.runway.app/certs/` on
