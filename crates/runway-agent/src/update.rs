...
fn installed_via_package_manager() -> bool {
    ...
    exe_str.starts_with("/usr/bin/") && !exe_str.contains(".runway/bin/")
}
...
async fn resolve_release(
    ...
    let asset_name = format!("runway-agent-linux-{arch}");
...
pub async fn run_update(version: Option<&str>, auto_yes: bool) -> anyhow::Result<()> {
    ...
    if installed_via_package_manager() {
        eprintln!("runway-agent was installed via a package manager.");
        eprintln!("Use your package manager to update (e.g., apt upgrade runway-agent).");
        std::process::exit(1);
    }

    let client = reqwest::Client::builder()
        .user_agent("runway-agent")
        .build()?;
