...
export default function TemplateStore({ onInstalled }: Props) {
  ...
  const [authTier, setAuthTier] = useState<AuthInfo["tier"]>("anonymous");
  const searchTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const { toast } = useToast();

  const canInstallPro = ["early_adopter", "pro", "team"].includes(authTier);
  ...
  const handleInstall = async (template: TemplateItem) => {
    if (template.pro_only && !canInstallPro) {
      toast(
        "This template requires Berth Pro. Upgrade at berth.sh/pro.",
        "error"
      );
      return;
    }
...
function TemplateCard({
  ...
  installing: boolean;
  canInstallPro: boolean;
  onInstall: () => void;
}) {
  const isLocked = template.pro_only && !canInstallPro;

  return (
    <div
      className={`rounded-lg border p-4 flex flex-col gap-2 transition-colors ${
        isLocked
          ? "border-berth-border/50 bg-berth-surface/50"
          : "border-berth-border bg-berth-surface hover:border-berth-accent/30"
      }`}
    >
      {/* Header row */}
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {template.pro_only && (
            <span className="shrink-0 inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold bg-berth-accent/10 text-berth-accent border border-berth-accent/20">
      ...
      <div className="flex items-center justify-between mt-auto pt-1">
        ...
        <button
          ...
          className={`px-3 py-1 rounded-md text-[11px] font-medium transition-colors ${
            isLocked
              ? "bg-berth-surface-2 text-berth-text-tertiary cursor-not-allowed"
              : "bg-berth-accent text-white hover:bg-berth-accent/90"
          }`}
        >
          {installing ? (
